    ...
    <div className="flex gap-2 flex-wrap">
      {CONTESTS.map(c => (
        <button
          ...
          className={`font-mono text-[0.65rem] tracking-[1.5px] uppercase px-3 py-1.5 rounded-sm border transition-all duration-150 ${
            selected === c.id
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span className={`font-mono text-[0.65rem] tracking-[2px] uppercase px-2.5 py-1 rounded-sm border ${
                contest.status === "live"      ? "text-acid border-acid/30 bg-acid/10"
          ...
          <div className="flex gap-1.5">
            {(["rank", "time", "completed"] as const).map(s => (
              <button
                key={s}
                onClick={() => setSortBy(s)}
                className={`font-mono text-[0.6rem] tracking-[1.5px] uppercase px-3 py-1.5 rounded-sm border transition-all ${
                  sortBy === s ? "bg-acid text-black border-acid" : "text-muted border-white/[0.08] hover:border-white/20 hover:text-cream"
                }`}
              >
                ...
                return (
                  <>
                    <tr
                      ...
                      className={`border-b border-white/[0.05] cursor-pointer transition-colors duration-150 group ${
                        p.isYou ? "bg-acid/[0.05]" : "hover:bg-white/[0.03]"
